...
<head>
    ...
    :root {
        ...
        --card-fill-top: rgba(255, 255, 255, 0.62);
        --card-fill-bottom: rgba(248, 242, 232, 0.34);
        --card-fill-hover-top: rgba(255, 255, 255, 0.74);
        --card-fill-hover-bottom: rgba(249, 243, 234, 0.44);
        --card-outline-soft: rgba(255, 255, 255, 0.62);
        --card-outline-mid: rgba(255, 245, 226, 0.24);
        --card-outline-hover-mid: rgba(255, 245, 226, 0.34);
        --card-outline-deep: rgba(158, 182, 208, 0.18);
        --card-outline-glow: rgba(255, 225, 163, 0.12);
        --card-sheen-top: rgba(255, 255, 255, 0.26);
        --card-sheen-bottom: rgba(255, 246, 231, 0.08);
        --card-side-accent: rgba(67, 184, 131, 0.78);
        --card-side-glow: rgba(255, 228, 177, 0.16);
        --card-backdrop-blur: 16px;
        --card-backdrop-saturate: 145%;
        --card-shadow: 0 8px 24px rgba(15, 23, 42, 0.08);
        --card-shadow-hover: 0 12px 28px rgba(15, 23, 42, 0.14);
    }
    ...
    body.dark-theme {
        ...
        --card-backdrop-saturate: 120%;
        --card-shadow: 0 10px 26px rgba(0, 0, 0, 0.22);
        --card-shadow-hover: 0 16px 34px rgba(0, 0, 0, 0.28);
    }
    ...
    #hitokoto {
        ...
        margin-left: auto;
        margin-right: auto;
        transition: opacity 0.32s ease, color 0.3s ease;
    }
    ...
    #dialog-box.is-busy .dialog-progress {
        display: flex;
    }
    ...
    .dialog-progress-spinner {
        width: 15px;
        height: 15px;
        border: 2px solid rgba(67, 184, 131, 0.25);
    ...
    body.dark-theme .add-dialog-title {
        color: #e3e3e3;
    }

    body.dark-theme .dialog-progress {
        color: #7bd9ad;
    }

    body.dark-theme .dialog-progress-spinner {
        border-color: rgba(123, 217, 173, 0.24);
        border-top-color: #7bd9ad;
    }
    ...
    .section.is-reorder-target {
        box-shadow: 0 18px 48px rgba(67, 184, 131, 0.13);
    }
    ...
    .card {
        ...
        box-shadow:
            ...
            0 12px 30px var(--card-outline-glow);
        backdrop-filter: blur(var(--card-backdrop-blur)) saturate(var(--card-backdrop-saturate));
        -webkit-backdrop-filter: blur(var(--card-backdrop-blur)) saturate(var(--card-backdrop-saturate));
        cursor: pointer;
    ...
    .card::before {
        ...
        pointer-events: none;
        background: linear-gradient(180deg, var(--card-sheen-top), transparent 42%, var(--card-sheen-bottom));
        opacity: 0.88;
    }
    ...
    .card::after {
        ...
        border-radius: 999px;
        pointer-events: none;
        background: linear-gradient(180deg, transparent, var(--card-side-accent), transparent);
        box-shadow: 0 0 14px var(--card-side-glow);
        opacity: 0.82;
    }
    ...
    .card:hover {
        ...
        background:
            linear-gradient(135deg, var(--card-fill-hover-top), var(--card-fill-hover-bottom)) padding-box,
            linear-gradient(145deg, var(--card-outline-soft), var(--card-outline-hover-mid) 34%, var(--card-outline-deep)) border-box;
        box-shadow:
            var(--card-shadow-hover),
            0 0 0 1px rgba(255, 255, 255, 0.05),
            0 14px 34px var(--card-outline-glow);
    }
    ...
    .move-category-btn:hover {
        background-color: #5473a9;
        transform: translateY(-1px);
        box-shadow: 0 6px 14px rgba(93, 127, 185, 0.2);
    }
...
<body>
    ...
    <div class="content">
        ...
        <div id="dialog-overlay">
            <div id="dialog-box">
                <h3 class="add-dialog-title" id="add-dialog-title">添加链接</h3>
                ...
                <div class="dialog-progress" id="dialog-progress" aria-live="polite">
                    <span class="dialog-progress-spinner" aria-hidden="true"></span>
                    <span id="dialog-progress-text">正在保存...</span>
                </div>
    ...
    function clearAuthToken() {
        ...
        localStorage.removeItem(LOGIN_TOKEN_EXPIRES_AT_KEY);
    }

    function saveAuthToken(token, expiryMinutes) {
        const safeExpiryMinutes = Number.isInteger(expiryMinutes) ? expiryMinutes : LOGIN_TOKEN_EXPIRY_MINUTES;
        localStorage.setItem(LOGIN_TOKEN_STORAGE_KEY, token);
        if (safeExpiryMinutes > 0) {
            localStorage.setItem(LOGIN_TOKEN_EXPIRES_AT_KEY, String(Date.now() + safeExpiryMinutes * 60 * 1000));
        } else {
            localStorage.removeItem(LOGIN_TOKEN_EXPIRES_AT_KEY);
        }
    }
    ...
    function getAuthToken() {
        ...
        if (!token) return '';
        const expiresAt = Number(localStorage.getItem(LOGIN_TOKEN_EXPIRES_AT_KEY) || '0');
    ...
    window.addEventListener('error', function(event) {
        ...
        console.error('前端脚本错误:', message);
        if (!mainAppBooted) {
            setBootStatus('前端脚本执行失败：' + message + '。请检查 Worker 控制台日志。', true);
        }
    });

    window.addEventListener('unhandledrejection', function(event) {
        const reason = event.reason && event.reason.message ? event.reason.message : String(event.reason || '未知错误');
        console.error('页面异步任务失败:', reason);
        if (!mainAppBooted) {
    ...
    function getSectionKey(section) {
        const cardContainer = section && section.querySelector('.card-container');
        return cardContainer ? cardContainer.id : '';
    }

    function captureSectionRects() {
        const rects = {};
        document.querySelectorAll('#sections-container .section').forEach(function(section) {
            const key = getSectionKey(section);
            if (key) {
                rects[key] = section.getBoundingClientRect();
            }
        });
        return rects;
    }

    function animateSectionReorder(previousRects, movedCategoryName) {
        ...
        sections.forEach(function(section) {
            ...
            section.classList.add('is-visible', 'is-reordering');
            if (key === movedCategoryName) {
                section.classList.add('is-reorder-target');
            }
            section.style.transition = 'none';
            section.style.transform = 'translate3d(' + deltaX + 'px, ' + deltaY + 'px, 0)';
        ...
        window.setTimeout(function() {
            animatedSections.forEach(function(section) {
                section.classList.remove('is-reordering', 'is-reorder-target');
                section.style.transition = '';
                section.style.transform = '';
            });
        }, 620);
    ...
    async function moveCategory(categoryName, direction) {
        ...
        const previousRects = captureSectionRects();
    ...
    // 保存链接数据
    async function saveLinks(options = {}) {
        const shouldValidate = options.skipValidation !== true;
        if (shouldValidate && isAdmin && !(await validateToken())) {
            return false;
        }

        const authToken = getAuthToken();
        if (!authToken) {
            const message = '登录状态无效，请重新登录';
            if (options.throwOnError) throw new Error(message);
            if (!options.silent) await customAlert(message, '保存失败');
            return false;
        }
        ...
        try {
            ...
            if (!response.ok || (result && result.success === false)) {
                const message = (result && (result.message || result.error)) || ('保存失败，HTTP ' + response.status);
            ...
            const message = error && error.name === 'AbortError'
                ? '保存超时，请检查网络后重试'
                : ((error && error.message) || '保存链接失败，请重试');
            logAction('保存链接失败', { error: message });
            console.error(message);
            if (options.throwOnError) {
                throw new Error(message);
            }
            if (!options.silent) {
                await customAlert(message, '保存失败');
            }
            return false;
        }
    }
    ...
    async function addLink() {
        ...
        const previousPublicLinks = [...publicLinks];
        const previousPrivateLinks = [...privateLinks];
        const previousLinks = [...links];
        const previousCategories = JSON.parse(JSON.stringify(categories));
        const restorePreviousState = () => {
            publicLinks = previousPublicLinks;
            privateLinks = previousPrivateLinks;
            links = previousLinks;
            Object.keys(categories).forEach(key => delete categories[key]);
            Object.assign(categories, previousCategories);
            updateCategorySelect();
            renderSections();
        };

        try {
            setAddDialogBusy(true, '正在验证登录状态...');
            if (!await validateToken()) {
            ...
            setAddDialogBusy(true, '正在保存到云端...');
            await saveLinks({ skipValidation: true, throwOnError: true, silent: true });

            setAddDialogBusy(true, '保存成功，正在刷新...');
            updateCategorySelect();
            renderSections();
            ...
            logAction('添加卡片', { name, url: normalizedUrl, tips, icon, category, isPrivate });
        } catch (error) {
            restorePreviousState();
            const message = (error && error.message) || '添加失败，请重试';
            logAction('添加卡片失败', { name, url: normalizedUrl, category, error: message });
            await customAlert(message, '添加卡片');
        } finally {
            setAddDialogBusy(false);
        }
    }
    ...
    async function performLogin() {
        ...
        try {
            ...
            if (result.valid) {
                isLoggedIn = true;
                saveAuthToken(result.token, result.expiryMinutes || LOGIN_TOKEN_EXPIRY_MINUTES);
                console.log('Token saved for ' + (result.expiryMinutes || LOGIN_TOKEN_EXPIRY_MINUTES) + ' minutes');
                loadLinks();
                hideLoginModal();
                updateLoginButton();
    ...
    let addDialogBusy = false;
    let addDialogCloseTimer = null;

    function openAddDialogOverlay() {
        const overlay = document.getElementById('dialog-overlay');
        if (!overlay) return;
        if (addDialogCloseTimer) {
    ...
    function closeAddDialogOverlay() {
        const overlay = document.getElementById('dialog-overlay');
        if (!overlay) return;
        overlay.classList.remove('is-open');
        if (addDialogCloseTimer) clearTimeout(addDialogCloseTimer);
    ...
    function setAddDialogBusy(isBusy, message) {
        addDialogBusy = !!isBusy;
        const dialogBox = document.getElementById('dialog-box');
        const confirmBtn = document.getElementById('dialog-confirm-btn');
        const cancelBtn = document.getElementById('dialog-cancel-btn');
        const progressText = document.getElementById('dialog-progress-text');
        if (dialogBox) dialogBox.classList.toggle('is-busy', addDialogBusy);
        if (confirmBtn) {
            confirmBtn.disabled = addDialogBusy;
            confirmBtn.textContent = addDialogBusy ? '处理中...' : '确定';
        }
        if (cancelBtn) cancelBtn.disabled = addDialogBusy;
    ...
    function showEditDialog(link) {
        setAddDialogBusy(false);
        openAddDialogOverlay();

        const dialogTitle = document.getElementById('add-dialog-title');
        if (dialogTitle) dialogTitle.textContent = '编辑链接';
        ...
        currentConfirmHandler = async function (event) {
            event.preventDefault();
            event.stopPropagation();
            if (!addDialogBusy) await updateLink(link);
        };
    ...
    function showAddDialog() {
        ...
        const dialogTitle = document.getElementById('add-dialog-title');
        if (dialogTitle) dialogTitle.textContent = '添加链接';
        ...
        currentConfirmHandler = async function (event) {
            event.preventDefault();
            event.stopPropagation();
            if (!addDialogBusy) await addLink();
        };
...
async function validateServerToken(authToken, env) {
    ...
    try {
        ...
        if (parts.length === 3) {
            ...
            expiryMinutes = isNaN(envExpiry) ? DEFAULT_TOKEN_EXPIRY_MINUTES : envExpiry;
        } else {
    ...
    async fetch(request, env) {
      ...
      if (url.pathname === '/api/saveOrder' && request.method === 'POST') {
        ...
        try {
            ...
            await withTimeout(env.CARD_ORDER.put(userId, JSON.stringify({ links, categories })), 8000, 'card_order_put_timeout');
            return new Response(JSON.stringify({
                success: true,
                message: '保存成功'
            }), {
            ...
            const isTimeout = error && error.message === 'card_order_put_timeout';
            return new Response(JSON.stringify({
                success: false,
                error: isTimeout ? 'card_order_put_timeout' : 'card_order_put_failed',
                message: isTimeout ? '保存超时，请稍后重试' : '保存失败，请稍后重试'
            }), {
      ...
      if (url.pathname === '/api/verifyPassword' && request.method === 'POST') {
        try {
            ...
            if (isValid) {
                ...
                const defaultExpiry = parseInt(env.TOKEN_EXPIRY_MINUTES) || DEFAULT_TOKEN_EXPIRY_MINUTES;
